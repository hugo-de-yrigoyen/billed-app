...
export default class {
  ...
  handleEditTicket(e, bill, bills) {
    const $card = $(`#open-bill${bill.id}`);
    const isAlreadySelected = $card.hasClass("selected");
    ...
    if (!isAlreadySelected) {
      $card.addClass("selected").css({ background: "#2A2B35" });
  ...
  handleShowTickets(e, bills, index) {
    const isOpen = this.statusListsOpened[index];
    ...
    this.statusListsOpened[index] = !isOpen;
    return bills;
  }
